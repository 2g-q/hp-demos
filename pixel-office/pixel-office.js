window.SCENE = {"meta":{"title":"ドット絵オフィス(見本)","client":"架空の会社・見本"},"canvas":{"w":640,"h":400,"scale":2},"palette":{"black":"#111827","white":"#F9FAFB","charcoal":"#374151","gray":"#9CA3AF","accent":"#10B981"},"clock":{"dayMs":240000,"startHour":9},"labels":{"walkOut":"退勤中","walkIn":"出勤中"},"characters":{"cio":{"label":"責任者が全体を確認しています","labels":{"idea":"方針が固まりました","thinking":"方針を検討しています","overview":"チームの状況を確認しています"},"look":{"shape":1,"hair":"#111827","shirt":"#F9FAFB","tie":true}},"audit":{"label":"品質を確認しています","labels":{"consult":"経理担当と確認しています","late":"最終確認を進めています"},"look":{"shape":2,"hair":"#374151","shirt":"#9CA3AF","paper":true}},"keiri":{"label":"収支を確認しています","labels":{"consult":"品質担当と確認しています","late":"月次集計を進めています"},"look":{"shape":3,"hair":"#111827","shirt":"#F9FAFB","female":true}},"eng":{"label":"機能を開発しています","labels":{"sync":"仕様を確認しています","late":"リリース対応中","test":"動作を検証しています"},"look":{"shape":0,"hair":"#374151","shirt":"#10B981","hoodie":true}},"eng2":{"label":"設計を進めています","labels":{"meeting":"企画担当と打ち合わせ中","consultOut":"設計相談へ向かっています","return":"開発席へ戻っています","late":"設計を仕上げています"},"look":{"shape":2,"hair":"#111827","shirt":"#9CA3AF","hoodie":true,"female":true}},"eng3":{"label":"画面を実装しています","labels":{"sync":"仕様をすり合わせています","late":"公開準備中","debug":"不具合を調べています"},"look":{"shape":3,"hair":"#374151","shirt":"#10B981","hoodie":true}},"eng4":{"label":"変更内容を確認しています","labels":{"thinking":"改善案を考えています","code":"修正を反映しています"},"look":{"shape":1,"hair":"#111827","shirt":"#374151","hoodie":true,"female":true}},"writer":{"label":"企画をまとめています","labels":{"meeting":"開発担当と打ち合わせ中","thinking":"構成を考えています"},"look":{"shape":0,"hair":"#374151","shirt":"#F9FAFB","female":true}},"carrier":{"label":"次の書類を取りにいきます","labels":{"sorting":"書類を整理しています","approved":"確認済み書類を運んでいます","received":"受領書類を運んでいます","delivering":"担当者へ書類を届けています","fetch":"書類を受け取りにいきます"},"look":{"shape":3,"hair":"#111827","shirt":"#F9FAFB","female":true}},"coffee":{"label":"飲み物を運んでいます","labels":{"serving":"チームへ飲み物を配っています","planning":"商談内容を確認しています","brewing":"飲み物を用意しています","toCounter":"休憩スペースへ向かっています"},"look":{"shape":1,"hair":"#111827","shirt":"#10B981","tie":true}},"sofa1":{"label":"休憩しています","labels":{"consult":"採用計画を相談しています","reading":"応募資料を確認しています"},"look":{"shape":2,"hair":"#374151","shirt":"#9CA3AF","female":true}},"sofa2":{"label":"会議資料を確認しています","labels":{"board":"予定表を確認しています","return":"打ち合わせ席へ戻っています","toBoard":"予定表を見にいきます","meeting":"採用担当と打ち合わせ中"},"look":{"shape":0,"hair":"#111827","shirt":"#F9FAFB","papers":true}},"break1":{"label":"休憩中","look":{"shape":3,"hair":"#374151","shirt":"#10B981","female":true}},"break2":{"label":"新しい施策を相談しています","look":{"shape":2,"hair":"#111827","shirt":"#F9FAFB","tie":true}}}};
/*
...
(function () {
  ...
  var canvasConfig = SCENE.canvas || {};
  var paletteConfig = SCENE.palette || {};
  var clockConfig = SCENE.clock || {};
  var labelsConfig = SCENE.labels || {};
  var charactersConfig = SCENE.characters || {};
  // 書いていない項目は既定値のまま。0やfalseも有効な指定になり得るため、
  // truthy判定でなく「キーがあるかどうか」(own)で差し替える。ownは関数宣言なので巻き上げで使える。
  function pick(obj, key, fallback) { return own(obj, key) ? obj[key] : fallback; }
  var W = pick(canvasConfig, "w", 640), H = pick(canvasConfig, "h", 400), SCALE = pick(canvasConfig, "scale", 2);
  var C = {
    black: pick(paletteConfig, "black", "#0D0D0D"),
    white: pick(paletteConfig, "white", "#FFFFFF"),
    charcoal: pick(paletteConfig, "charcoal", "#343541"),
    gray: pick(paletteConfig, "gray", "#8E8EA0"),
    blue: pick(paletteConfig, "accent", "#2563EB")
  };
  ...
  function configuredCharacter(id) {
    if (!Array.isArray(charactersConfig)) return charactersConfig[id];
    var roleFor = { eng: "engineer", eng2: "engineer2", eng3: "engineer3", eng4: "engineer4" };
    for (var i = 0; i < charactersConfig.length; i += 1) {
      if (charactersConfig[i].id === id || charactersConfig[i].role === (roleFor[id] || id)) return charactersConfig[i];
    }
    return null;
  }
  function configuredColor(token, fallback) {
    // パレットのキー名(black/white/charcoal/gray/accent)か混色指定
    // ({"mix":[キー,キー,割合]} または "キー+キー@割合")だけを受ける。
    // 生の色コード(hex等)はどの形にも合わず既定色へ落ちる=
    // 「指定5色とその混色だけ」の規律をシーン側から破れない。
    var names = { black: C.black, white: C.white, charcoal: C.charcoal, gray: C.gray, accent: C.blue };
    if (typeof token === "string" && names[token]) return names[token];
    if (token && typeof token === "object" && Array.isArray(token.mix) && token.mix.length === 3 &&
        names[token.mix[0]] && names[token.mix[1]] && typeof token.mix[2] === "number") {
      return mix(names[token.mix[0]], names[token.mix[1]], token.mix[2]);
    }
    var match = /^([a-z]+)\+([a-z]+)@([01](?:\.\d+)?)$/.exec(typeof token === "string" ? token : "");
    return match && names[match[1]] && names[match[2]] ? mix(names[match[1]], names[match[2]], Number(match[3])) : fallback;
  }
  function sceneStyle(id, fallback) {
    var char = configuredCharacter(id), look = char && char.look;
    if (!look) return fallback;
    var out = {}, key;
    for (key in fallback) if (own(fallback, key)) out[key] = fallback[key];
    // shape=0やfalseも有効な指定なので、truthy判定ではなく「キーがあるか」で差し替える。
    if (own(look, "shape")) out.shape = look.shape;
    if (own(look, "hair")) out.hairC = configuredColor(look.hair, fallback.hairC);
    if (own(look, "shirt")) out.shirtC = configuredColor(look.shirt, fallback.shirtC);
    ["tie", "female", "hoodie", "paper", "papers"].forEach(function (flag) {
      if (own(look, flag)) out[flag] = look[flag];
    });
    return out;
  }
  function sceneLabel(id, key, fallback) {
    var char = configuredCharacter(id);
    if (!char) return fallback;
    if (char.labels && own(char.labels, key)) return char.labels[key];
    // labelは文言の基本形: 指定されたら状況分岐をやめてlabel一本にする。
    // 帰宅・出社中(walkOut/walkIn)だけは labels.walkOut/walkIn 側の文言のまま。
    if (key !== "walkOut" && key !== "walkIn" && own(char, "label")) return char.label;
    return fallback;
  }
  ...
  // 0時(startHour:0)も有効な指定=キーの有無で差し替える。
  var DAY_MS = pick(clockConfig, "dayMs", 180000), START_HOUR = pick(clockConfig, "startHour", 8);
  ...
  ST_CIO = sceneStyle("cio", ST_CIO); ST_AUDIT = sceneStyle("audit", ST_AUDIT);
  ST_KEIRI = sceneStyle("keiri", ST_KEIRI); ST_ENG = sceneStyle("eng", ST_ENG);
  ST_ENG2 = sceneStyle("eng2", ST_ENG2); ST_ENG3 = sceneStyle("eng3", ST_ENG3);
  ST_ENG4 = sceneStyle("eng4", ST_ENG4); ST_WRITER = sceneStyle("writer", ST_WRITER);
  ST_CARRY = sceneStyle("carrier", ST_CARRY); ST_COFFEE = sceneStyle("coffee", ST_COFFEE);
  ST_SOFA = sceneStyle("sofa1", ST_SOFA); ST_SOFA2 = sceneStyle("sofa2", ST_SOFA2);
  ST_SOFA2W = sceneStyle("sofa2", ST_SOFA2W); ST_BREAK1 = sceneStyle("break1", ST_BREAK1);
  ST_BREAK2 = sceneStyle("break2", ST_BREAK2);
  // 離席中はheldItem側で書類を描く。lookのpapersを残すと二重になる罠があるため従来どおり外す。
  delete ST_SOFA2W.paper; delete ST_SOFA2W.papers;
  // present:false はその役割を描かない(椅子・机・小物は残る=座っていないだけ)。
  // 歩き回る役割はその人物の描画と当たり判定だけを止める(経路・周期の仕組みは触らない)。
  // キーがあるかどうかで差し替える(present:true の明示も有効)。
  function scenePresent(id) {
    var ch = configuredCharacter(id);
    return ch && own(ch, "present") ? !!ch.present : true;
  }
  var PRESENT = {
    cio: scenePresent("cio"), audit: scenePresent("audit"), keiri: scenePresent("keiri"),
    eng: scenePresent("eng"), eng2: scenePresent("eng2"), eng3: scenePresent("eng3"),
    eng4: scenePresent("eng4"), writer: scenePresent("writer"), carrier: scenePresent("carrier"),
    coffee: scenePresent("coffee"), sofa1: scenePresent("sofa1"), sofa2: scenePresent("sofa2"),
    break1: scenePresent("break1"), break2: scenePresent("break2")
  };
  ...
  window.mountSchemePixel = function mountSchemePixel(canvas) {
    ...
    function deskTowards(x, yd, ms, phase) {
      ...
      chairTop(x + 5, py);
      if (PRESENT.audit) person(x + 5, py, chat ? "right" : "down", f1, ST_AUDIT, true);
      chairTop(x + 51, py);
      if (PRESENT.keiri) person(x + 51, py, chat ? "left" : "down", f2, ST_KEIRI, true);
    ...
    function deskAway(x, yd, ms, phase, hideSecond) {
      ...
      chairFront(x + 5, yc);
      if (PRESENT.eng) person(x + 5, py, "up", f1, ST_ENG, true);
      chairBack(x + 5, yc);
      chairFront(x + 51, yc);
      if (!hideSecond && PRESENT.eng2) person(x + 51, py, "up", f2, ST_ENG2, true); // 離席中は椅子だけ
      chairBack(x + 51, yc);
    }
    ...
    function deskFacing(x, yd, ms) {
      ...
      var py = yd - SIT_FRONT;
      chairTop(x + 5, py);
      if (PRESENT.eng3) person(x + 5, py, "down", f1, ST_ENG3, true);
      chairTop(x + 51, py);
      if (PRESENT.eng4) person(x + 51, py, "down", f2, ST_ENG4, true);
      deskBlock(x, yd, DESK_W);
    ...
    function draw(ms) {
      ...
      var caps = [], feet = [];
      var carrier = PRESENT.carrier ? resolveActor(carrierState, ms, OBST_MAIN) : null;
      if (carrier) {
        feet.push(footOf(carrier.x, carrier.y));
        caps.push({ foot: feet[0], obst: OBST_MAIN, moving: !!carrier.moving });
      }
      var coffee = PRESENT.coffee ? resolveActor(coffeeFull, ms, OBST_MAIN.concat(feet)) : { gone: true };
      if (!coffee.gone) {
        feet.push(footOf(coffee.x, coffee.y));
        caps.push({ foot: footOf(coffee.x, coffee.y), obst: OBST_MAIN, moving: !!coffee.moving });
      }
      var talk = discussState(ms), talkOut = talk.mode !== "seat" && PRESENT.eng2;
      if (talkOut) {
      ...
      if (!PRESENT.sofa2) sofaSc.away = false; // 不在なら離席の歩きも出さない(座面は空のまま)
      if (sofaSc.away) {
      ...
      if (!PRESENT.cio) cioMode = "away"; // 不在=描かない(ハイバックの椅子だけ残る)
      ...
      if (!PRESENT.break1) duoAMode = "away"; // 不在=描かない
      ...
      if (!PRESENT.break2) duoBMode = "away"; // 不在=描かない
      ...
      if (PRESENT.writer) add(343, function () { boardWriter(ms, talk.mode === "talk" && PRESENT.eng2); });
      ...
      if (sofaSc.talking && PRESENT.sofa1 && PRESENT.sofa2 && tb >= 0 && (!quiet || Math.floor((ms + 3700) / CONV_MS) % 3 === 0)) {
        bubble(tb === 0 ? 344 : 390, 228, (Math.floor((ms + 3700) / CONV_MS) * 2 + tb + 1) % 3);
      }
      if (talk.mode === "talk" && PRESENT.eng2 && PRESENT.writer) {
      ...
      if (chatNow.pair === "audit") { // 監査⇄経理: 横を向いて3.5秒交代で1往復
        ...
        if (PRESENT.audit && PRESENT.keiri) bubble(ca === 0 ? 95 : 141, 104, (Math.floor(chatNow.t / 3500) + 1) % 3);
      } else if (chatNow.pair === "engv") { // 向かい合わせのエンジニア(上下の席)
        var ce = Math.floor(chatNow.t / 3500) % 2;
        if (PRESENT.eng && PRESENT.eng3) bubble(66, ce === 0 ? 166 : 236, (Math.floor(chatNow.t / 3500) + ce) % 3);
        ...
        if (chatNow.t < 3500) { if (PRESENT.writer) thinkBubble(238, 258, false, ms); }
        else if (PRESENT.eng4) thinkBubble(112, 166, chatNow.t >= 5500, ms);
      }
      ...
      if (cioHere) {
        addHit("cio", 306, 128, 46, 206,
          chatNow.pair === "cio"
            ? (chatNow.t >= 9000 ? sceneLabel("cio", "idea", "ひらめきました")
              : chatNow.t >= 3000 && chatNow.t < 6000
                ? sceneLabel("cio", "thinking", "考えています")
                : sceneLabel("cio", "overview", "フロアを見渡しています"))
            : sceneLabel("cio", "default", "全体を見ています"));
      } else if (cioMode === "out" || cioMode === "in") {
        addHit("cio", cio.x, cio.y, UNIT, cio.y + UNIT,
          cioMode === "out"
            ? sceneLabel("cio", "walkOut", pick(labelsConfig, "walkOut", "帰宅中"))
            : sceneLabel("cio", "walkIn", pick(labelsConfig, "walkIn", "出社中")));
      }
      if (PRESENT.audit) addHit("audit", 101, 128, 46, 206,
        chatNow.pair === "audit" ? sceneLabel("audit", "consult", "隣と相談しています")
          : late ? sceneLabel("audit", "late", "夜通し見直しています")
            : sceneLabel("audit", "default", "見直しています"));
      if (PRESENT.keiri) addHit("keiri", 147, 128, 46, 206,
        chatNow.pair === "audit" ? sceneLabel("keiri", "consult", "隣と相談しています")
          : late ? sceneLabel("keiri", "late", "夜通し数字を合わせています")
            : sceneLabel("keiri", "default", "数字を合わせています"));
      if (PRESENT.eng) addHit("eng", 69, 256, 48, 319,
        chatNow.pair === "engv" ? sceneLabel("eng", "sync", "向かいと認識合わせ中")
          : late ? sceneLabel("eng", "late", "夜通しコードを書いています")
            : (ms % 42000 < 21000 ? sceneLabel("eng", "default", "コードを書いています")
              : sceneLabel("eng", "test", "テスト中")));
      if (PRESENT.eng3) addHit("eng3", 69, 187, 46, 265,
        chatNow.pair === "engv" ? sceneLabel("eng3", "sync", "向かいと認識合わせ中")
          : late ? sceneLabel("eng3", "late", "夜通し実装中")
            : (ms % 36000 < 18000 ? sceneLabel("eng3", "default", "実装中")
              : sceneLabel("eng3", "debug", "デバッグ中")));
      if (PRESENT.eng4) addHit("eng4", 115, 187, 46, 265,
        chatNow.pair === "think" && chatNow.t >= 3500 ? sceneLabel("eng4", "thinking", "考えています")
          : ms % 38000 < 19000 ? sceneLabel("eng4", "default", "レビュー中")
            : sceneLabel("eng4", "code", "コードを書いています"));
      if (duoAHere) addHit("break1", 458, 308, UNIT, 359,
        sceneLabel("break1", "default", "コーヒーブレイク中"));
      else if (duoAMode === "out" || duoAMode === "in") {
        addHit("break1", duoA.x, duoA.y, UNIT, duoA.y + UNIT,
          duoAMode === "out"
            ? sceneLabel("break1", "walkOut", pick(labelsConfig, "walkOut", "帰宅中"))
            : sceneLabel("break1", "walkIn", pick(labelsConfig, "walkIn", "出社中")));
      }
      if (duoBHere) addHit("break2", 508, 308, UNIT, 359,
        sceneLabel("break2", "default", "アイデアを出し合っています"));
      else if (duoBMode === "out" || duoBMode === "in") {
        addHit("break2", duoB.x, duoB.y, UNIT, duoB.y + UNIT,
          duoBMode === "out"
            ? sceneLabel("break2", "walkOut", pick(labelsConfig, "walkOut", "帰宅中"))
            : sceneLabel("break2", "walkIn", pick(labelsConfig, "walkIn", "出社中")));
      }
      if (talkOut) {
        addHit("eng2", talk.x, talk.y, UNIT, talk.y + UNIT,
          talk.mode === "talk" ? sceneLabel("eng2", "meeting", "打ち合わせ中")
            : talk.mode === "out" ? sceneLabel("eng2", "consultOut", "PCを持って相談にいきます")
              : sceneLabel("eng2", "return", "席に戻ります"));
      } else if (PRESENT.eng2) {
        addHit("eng2", 115, 256, 48, 319,
          late ? sceneLabel("eng2", "late", "夜通し設計中") : sceneLabel("eng2", "default", "設計中"));
      }
      if (PRESENT.writer) addHit("writer", 234, 292, UNIT, 343,
        talk.mode === "talk" ? sceneLabel("writer", "meeting", "打ち合わせ中")
          : chatNow.pair === "think" && chatNow.t < 3500 ? sceneLabel("writer", "thinking", "考えています")
            : sceneLabel("writer", "default", "構想を練っています"));
      if (carrier) addHit("carrier", carrier.x, carrier.y, UNIT, carrier.y + UNIT,
        carrier.sorting ? sceneLabel("carrier", "sorting", "資料を整理しています")
          : carrier.carry === "folder" ? sceneLabel("carrier", "approved", "承認済みの資料を持ち帰っています")
            : carrier.carry === "papers"
              ? (carrier.fetch ? sceneLabel("carrier", "received", "受け取った資料を運んでいます")
                : sceneLabel("carrier", "delivering", "資料を届けています"))
              : carrier.fetch ? sceneLabel("carrier", "fetch", "資料を受け取りにいきます")
                : sceneLabel("carrier", "default", "次の資料を取りにいきます"));
      if (!coffee.gone) {
        addHit("coffee", coffee.x, coffee.y, UNIT, coffee.y + UNIT,
          coffee.leaving ? sceneLabel("coffee", "walkOut", pick(labelsConfig, "walkOut", "帰宅中"))
            : coffee.arriving ? sceneLabel("coffee", "walkIn", pick(labelsConfig, "walkIn", "出社中"))
              : coffee.serving ? sceneLabel("coffee", "serving", "コーヒーを配っています")
                : coffee.home ? sceneLabel("coffee", "planning", "商談の作戦を練っています")
                  : coffee.atCounter ? sceneLabel("coffee", "brewing", "コーヒーを淹れています")
                    : coffee.cup ? sceneLabel("coffee", "default", "コーヒーを運んでいます")
                      : sceneLabel("coffee", "toCounter", "コーヒーを淹れにいきます"));
      }
      if (PRESENT.sofa1) addHit("sofa1", 339, 260, 44, 324,
        sofaSc.talking ? sceneLabel("sofa1", "consult", "採用の相談中")
          : sofaSc.aTablet ? sceneLabel("sofa1", "reading", "資料に目を通しています")
            : sceneLabel("sofa1", "default", "ひと息ついています"));
      if (sofaSc.away) {
        addHit("sofa2", sofaWalk.x, sofaWalk.y, UNIT, sofaWalk.y + UNIT,
          sofaWalk.mode === "board" ? sceneLabel("sofa2", "board", "ボードを確認しています")
            : sofaWalk.mode === "back" ? sceneLabel("sofa2", "return", "ソファに戻ります")
              : sceneLabel("sofa2", "toBoard", "ボードを確認しにいきます"));
      } else if (PRESENT.sofa2) {
        addHit("sofa2", 386, 260, 44, 324,
          sofaSc.talking ? sceneLabel("sofa2", "meeting", "打ち合わせ中")
            : sceneLabel("sofa2", "default", "資料を確認しています"));
      }
      drawTips(ms);
    }
    ...
    function sofaUnit(ms, sc) { // ソファの2人: 姿勢は予定表(sofaScene)から決める
      ...
      if (!sc.away && PRESENT.sofa2) { // 離席中は空いた座面だけが残る
